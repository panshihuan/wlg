                           ...
                           if(!resd.factoryStationGidRef){
                            ...
                            if(!reg.test(resd.materialNumber)){
                              ...
                              pubsub.publish("@@message.error","输入的数量必须小于拆分前的数量,请重新输入")
                                ...
                                let index 
                                let resda = resd
                          resolveFetch({fetch:{id:'tafnjnjdkkdll0jsodojw4jh777frr',data:'dataContext'}}).then(function(res){
                            index = res
                          })
                          resolveFetch({fetch:{id:'modify',data:'@@formValues'}}).then(function(res){
                            const actualQty = res.actualQty
                            res.imeWorkOrderDetailDTOs.splice(index,1)
                            res.imeWorkOrderDetailDTOs.push(saveData)
                            delete resd.gid
                            res.imeWorkOrderDetailDTOs.push(resd)
                            for(var i=0;i<res.imeWorkOrderDetailDTOs.length;i++){
                              res.imeWorkOrderDetailDTOs[i].planQty = parseInt(res.imeWorkOrderDetailDTOs[i].materialNumber) / parseInt(res.imeWorkOrderDetailDTOs[i].productNumber)
                              res.imeWorkOrderDetailDTOs[i].orderQty = parseInt(res.imeWorkOrderDetailDTOs[i].planQty) * parseInt(actualQty)
                            }
                            pubsub.publish("@@form.change", { id: "modify",name:"imeWorkOrderDetailDTOs" ,value: fromJS(res.imeWorkOrderDetailDTOs) })
