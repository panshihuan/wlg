                              ...
                              behaviors: [
                                  {
                                      ...
                                      resolveFetch({fetch: {id: "Create",data: "@@formValues"}}).then(function(data){
                                           defOperationDto = data;
                                      })
                                      resolveFetch({fetch: {id: "tafnjnjjsodojw4jhg7frr", data: "state"}}).then(function(tableData){
                                            mdEquipmentData = tableData.dataSource;
                                      })
                                      let params = {
                                        defOperation: defOperationDto,
                                        mdEquipment: mdEquipmentData
                                      }
                  ...
                  <Col span={6}>
                      <Field config={{
                          ...
                          subscribes: [
                            ...
                            pubsub.publish("@@form.change", { id: "Create", name: "mdFactoryWorkUnitName", value: val })
                            pubsub.publish("@@form.change", { id: "Create", name: "mdFactoryStationName", value: val })
                          `
                                          }
                                      }
                                  ]
                              ...
                              /* {
                                  event: "mdFactoryStationName.onChange",
                                  pubs:[
                                      {
                                          event:"mdFactoryLineName.expression",
                                          meta: {
                                              expression: `
                                  resolveFetch({fetch:{id:'Create',data:'@@formValues'}}).then(function(data){
                                    let workUnitGid = data.mdFactoryWorkUnitGid
                                    //console.log(workUnitGid)
                                    let dataSource = {
                                        type: "api",
                                        mode:"dataContext",
                                        method: "POST",
                                        url: "/ime/mdFactoryWorkUnit/findById.action?id="+workUnitGid,
                                    }
                                    resolveDataSource( {dataSource:dataSource, dataContext: { id: workUnitGid }}).then(function (unitData) {
                                        //console.log(unitData)
                                        if(unitData.data.factoryLineGidRef){
                                            let lineName = unitData.data.factoryLineGidRef.lineName
                                            let lineGid = unitData.data.factoryLineGidRef.gid
                                            data.mdFactoryLineGid = lineGid
                                            data.mdFactoryLineName = lineName
                                            pubsub.publish("@@form.change", { id: "Create", name: me.props.input.name, value: lineName })
                                            //console.log(data)
                                        }
